...
class GUI:
    def __init__(self, root, imsrc, lyrics_dir):
        self.root = root
        self.logo = imsrc
    ...
    def download(self):
        lyrics = self.lyrics_content.get('1.0', tk.END)
        song =   'by'.join(self.title_artist_label['text'].split('/'))
        with open(os.path.join(self.lyrics_dir, song) + '.txt', 'w') as file:
            for line in lyrics:
                file.write(line)
...
if __name__ == '__main__':
    ...
    lyricsfinderGUI = GUI(root, img, lyrics_dir)
    lyricsfinderGUI.run()